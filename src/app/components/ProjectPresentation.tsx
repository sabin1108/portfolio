...
const emphasisPhrases = ["action 슬롯", "게임 ID를 기준으로 중복 없이", "기존 카드는 유지", "value/onChange 기반 제어 컴포넌트","입력·초기화 UI는 공유","필요한 액션을 조합","Zustand","검색·태그·앨범 조건은 URL","모달·편집 입력은 local state", "첫 사진 표시 p95 약 2.6초", "59.6ms에서 13.2ms", "목록에는 작은 이미지를, 상세 화면에는 큰 이미지를", "높은 요청 우선순위는 첫 사진에만", "가상화 유무만 바꿨습니다", "CSS 좌표와 SVG 선 위치만 직접 바꿉니다", "기존 시뮬레이션을 중지", "각 화면이 필요한 상태만 selector로 구독", "Zustand로 전환", "공통 TypeScript 모델로 변환", "5초 제한", "이전 응답을 경고 정보와 함께 반환"];
...
export function ProjectPresentation({ project, index, notes }: { project: typeof frontendPortfolio.projects[number]; index: number; notes: Notes }) {
  ...
  const pages: { kind: string; label: string; content: ReactNode }[] = [
    { kind: "overview", label: "서비스 소개", content: <>
      ...
      {!photo && <dl className="bin-deck-highlights" aria-label="게임 할인 플랫폼 핵심 성과">{notes.cases.map(item => item.metric ? <div key={item.title}><dt>{item.metric.label}</dt><dd><strong>{item.metric.value}</strong><span>{item.metric.change}</span></dd></div> : null)}</dl>}
    ...
    ...notes.cases.flatMap((item, i) => [
      ...
      { kind: "story", label: `해결한 문제 0${i + 1} · 변경`, content: <><p className="bin-work-eyebrow">해결한 문제 0{i + 1} · 어떻게 바꿨나</p><h2>{photo ? ["크기에 맞게 요청하고,\n먼저 볼 사진부터.", "렌더링 범위를 줄여\n선택 반응을 확인했습니다."][i] : ["카드 렌더 호출을\n약 67% 줄였습니다.", "동시 요청 4건을\n후보 조회 1회로."][i]}</h2><ol className="bin-deck-process">{item.action.map((step, n) => <li key={step}><span>0{n + 1}</span><p><HighlightedCopy text={step} /></p></li>)}</ol><div className="bin-deck-result"><span>{item.metric?.label ?? "결과"}</span>{item.metric && <div className="bin-deck-metric"><strong>{item.metric.value}</strong><span>{item.metric.change}</span></div>}<p><HighlightedCopy text={item.result} /></p></div></> },
    ]),
    { kind: "evidence", label: "측정 근거와 적용 기술", content: <><p className="bin-work-eyebrow">측정 근거와 적용 기술</p><h2>{photo ? "확인한 조건까지 함께 남겼습니다." : "줄인 작업량과, 적용한 기술."}</h2><div className="bin-deck-evidence">{notes.cases.map((item, i) => <article key={item.title}><span>0{i + 1} · {photo ? ["이미지 측정", "목록 렌더링"][i] : ["카드 렌더 호출", "검색 후보 조회 공유"][i]}</span><p>{item.note}</p></article>)}{notes.implementation.map((part, i) => <article key={part.title}><span>0{i + 3} · {photo ? ["공통 UI·키보드 조작", "탐색 조건·목록 위치 복원"][i] : ["memo·이미지 로딩", "입력 분리·서버 스트리밍"][i]}</span><p>{part.note}</p></article>)}</div></> },
  ];
