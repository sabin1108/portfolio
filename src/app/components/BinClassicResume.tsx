  ...
  return selectedStories.map((story) => {
    if (projectTitle === "PhotoMap" && story.title === "모바일에서 첫 사진이 늦게 나타난 문제") {
      return {
        ...
        action: "목록에서는 사진을 작은 카드로 보여주기 때문에 원본 크기의 이미지가 필요하지 않았습니다. 목록에는 작은 이미지를, 상세 화면에는 큰 이미지를 불러오도록 나누고 이미지 주소를 선택하는 코드를 한곳에 모았습니다. 첫 줄의 사진은 바로 불러오되 높은 요청 우선순위는 첫 사진에만 지정하며, 나머지 사진은 화면에 가까워질 때 요청하도록 바꿔 초기 요청이 한꺼번에 몰리지 않게 했습니다. 작은 이미지가 없는 기존 사진은 원본 주소를 사용하도록 처리했습니다.",
