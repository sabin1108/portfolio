...
export const resumeData = {
  ...
  summary:
    `저는 넓게 탐색하되, 적용할 때는 근거를 기준으로 좁혀가는 프론트엔드 개발자 민사빈입니다. 궁금한 기술은 직접 파보고 배우지만, 실제 적용은 지표와 유지보수성을 기준으로 판단합니다.

PhotoMap에서는 Context API를 Zustand Selector 구조로 전환해 리렌더링 범위를 70여 개 컴포넌트에서 5개 이하 컴포넌트로 좁혔고, React Profiler 측정 기준 커밋 시간을 9.7ms에서 6.2ms로 약 36% 단축했습니다. @tanstack/react-virtual 기반 가상화로 DOM 노드를 약 200개 수준으로 유지해 대량 사진 구간의 스크롤 프레임 저하를 완화했고, D3 물리 시뮬레이션은 React State 대신 Ref와 DOM 직접 조작으로 처리해 리렌더링 횟수를 370회에서 25회로 줄였습니다. 여기에 더해 Mapbox 기반 지도 화면에 위치 데이터를 연결하고 Unity WebGL iframe과 postMessage로만 데이터를 주고받도록 분리해, 지도와 3D 렌더링, React 상태 변경이 서로의 책임을 침범하지 않는 구조를 만들었습니다.

Game Information Platform에서는 캐시 매니저 등 인프라 유틸리티에 제네릭을 적용해 여러 데이터 타입에 재사용 가능한 구조로 만들었습니다. CI에서는 typecheck, lint, Vitest 테스트, production build, Playwright smoke 테스트를 실행하도록 구성했고, 별도의 로컬 부하 테스트 스크립트로 공개 API의 status, cache, p50/p95 latency를 확인할 수 있게 했습니다. Fallow 정적 분석으로 중복 코드를 1,006줄(9.4%)에서 308줄(2.9%)로 줄이고, 미사용 파일/export를 0개로 정리해 유지보수성도 함께 관리했습니다.

저는 화면 구현을 넘어 아키텍처와 품질을 함께 설계하는 프론트엔드 엔지니어로 성장하고 싶습니다. 지표 기반으로 성능을 진단·개선하고 설계했던 경험을 바탕으로, 유지보수 가능한 코드베이스와 팀의 개발 표준까지 함께 만들어가고 싶습니다. 나아가 AI 도구를 단순 생성기가 아니라 요구사항 정리, 구현, 검증, 문서화 흐름을 보조하는 개발 프로세스의 일부로 활용해온 경험을 살려, AI 기반 기능도 제품 흐름 안에서 신뢰할 수 있게 녹여내는 엔지니어로 기여하고 싶습니다.`,
  coreSkills: [
