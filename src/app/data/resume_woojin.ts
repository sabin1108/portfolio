...
HTML/CSS/JavaScript 기반 화면 구현, API 경계 분리, 공공 정보 접근성 개선 챗봇, Mapbox·WebGL 기반 지도 데이터 탐색 UI 경험을 바탕으로 공공·국방 정보체계 웹 개발에 빠르게 적응하겠습니다.`,
  ...
  projectHighlights: resumeData.projectHighlights.map((project) => {
    if (project.title === "PhotoMap") {
      return {
        ...project,
        techTags: ["React", "TypeScript", "Zustand", "Mapbox", "Web GIS", "@tanstack/react-virtual", "Unity WebGL", "Canvas", "Lighthouse", "React Profiler"],
        ...
        resolutions: [
          ...
          "@tanstack/react-virtual 기반 row 가상화로 보이는 구간만 DOM에 유지하고, hover/transition 비용이 큰 이미지 카드는 메모이제이션해 대량 사진 구간의 스크롤 병목을 완화했습니다."
        ],
        achievements: [
          ...
          "row 가상화 적용 후 10,000건 이상의 대량 이미지 데이터에서도 DOM 노드를 약 200개 수준으로 유지해 브라우저가 감당해야 하는 렌더링 노드 수를 고정했습니다.",
