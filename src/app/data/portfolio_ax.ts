...
const photoMapCases: CaseStudy[] = [
  {
    ...
    cause: "기준 fixture는 grid와 detail에 같은 522,002B JPEG를 사용하고 여러 이미지 요청이 제한된 모바일 대역폭을 함께 점유했습니다. 작은 카드와 상세 화면의 전달 정책이 분리되지 않아 첫 LCP 후보가 나머지 요청과 경쟁했습니다.",
    resolution: "실제 사용자 데이터와 Production DB를 사용하지 않는 고정 Vercel Preview benchmark를 만들었습니다. 390×844, DPR 2, CPU 4배 slowdown, RTT 150ms, 다운로드 1.6Mbps, 업로드 0.75Mbps를 고정하고, 같은 commit·deployment에서 query parameter만 baseline/optimized로 바꿨습니다. optimized 100회와 baseline 30회를 매회 fresh Chromium context, concurrency 1로 실행했습니다.",
  ...
  },
  {
    ...
    result: "optimized cold run당 CDP 관측 전송량은 335,423B였고 첫 사진 완료 p95는 2.6초였습니다. 유료 Image Transformations runtime 의존을 제거하고 legacy record는 public original URL로 fallback했습니다. TypeScript, image delivery·traffic policy 테스트 8/8, production build를 통과했습니다.",
...
const photoMapAx: Project = {
    ...
    tracks: [
      {
        ...
        environment: "고정 Vercel Preview · Headless Chrome 151 / Playwright 1.62 · Windows 단일 physical runner · 390×844 / DPR 2 / mobile touch · CPU 4× slowdown · RTT 150ms · 1.6Mbps down / 0.75Mbps up · concurrency 1",
        dataset: "DB와 env에 의존하지 않는 synthetic fixture · baseline JPEG 522,002B · optimized thumbnail WebP 30,496B / display WebP 260,958B · 실제 사용자 사진과 Production DB/Storage 미사용",
        procedure: "동일 commit 3f0beac·deployment·browser profile 유지 · query parameter만 mode 변경 · optimized cold 100회 / baseline cold 30회 · 매회 fresh Chromium context · 이미지 완료 후 100ms settle하고 Performance Timeline의 마지막 LCP entry 수집",
        ...
        result: "optimized: lab LCP p75 2.5초 · 첫 사진 p95 2.6초 / max 2.7초 · 첫 사진·모달 100/100 성공 · 1,300 requests에서 request·HTTP·console 오류 0 · 관측 전송량 335,423B/run",
        limitation: "synthetic lab이며 field RUM 아님 · fresh context도 Vercel edge·DNS·OS cache는 공유 · 단일 runner 위치 미검증 · baseline 30회와 optimized 100회를 순차 실행",
      },
      {
        name: "실험 안전·무결성 검증",
        purpose: "Production 오염, 인증 화면 오인, 측정 race, 실패 표본 누락을 차단하고 결과를 다시 계산할 수 있게 보존",
        environment: "staging-only 정책 · HTTPS Vercel hostname과 app main 응답 preflight · Production Supabase ref 관측 시 즉시 중단 · benchmark fixture를 Supabase와 VITE_DEMO_USER_ID에서 격리",
        dataset: "각 cold·warm sample의 timing·성공 여부·request count·CDP bytes·오류 배열 · 실행 commit·branch·dirty state·Node·Chrome·Playwright·OS metadata · canonical raw JSON 2개",
        procedure: "실패도 sample 배열과 성공률 분모에 유지 · LCP observer race 발견 후 Timeline 직접 조회로 runner 수정 · 수정 전 100/30 결과 폐기 · 두 mode 전체 재실행 · raw SHA-256과 재현 명령 문서화",
        criteria: "configured count와 실제 sample count 일치 · 같은 commit hash · observedSupabaseHosts 빈 배열 · 첫 사진·모달 성공률 100% · request failure·HTTP·console 오류 0 · JSON parse 및 테스트·build 통과",
        result: "optimized 100/100, baseline 30/30 cold sample 보존 · 두 mode 모두 성공률 100% · 최종 오류 0/0/0 · image delivery·policy 테스트 8/8, TypeScript, production build 통과",
        limitation: "raw JSON의 gitDirty=true는 측정 중 생성 파일이 untracked였기 때문 · runner와 tracked application source는 기록된 commit과 일치 · 장기 결론은 7~14일 RUM 필요",
      },
    ],
  },
  metricRows: [
    ...
    { category: "Traffic", metric: "관측 전송량 / run", before: "3,200,451B", after: "335,423B", basis: "CDP encodedDataLength" },
  ...
  architecture: {
    ...
    columns: [
      { title: "격리 대상", nodes: [{ label: "Preview", detail: "고정 Vercel deployment" }, { label: "Fixture", detail: "DB·env 비의존 합성 이미지" }] },
      ...
      { title: "Evidence", nodes: [{ label: "Metrics", detail: "LCP·첫 사진·bytes·오류" }, { label: "Raw", detail: "sample JSON·환경 meta·SHA-256" }] },
    ],
    flow: ["Production과 분리된 고정 Preview와 synthetic fixture를 만듭니다.", "기기·네트워크·commit을 고정하고 mode만 바꿔 반복 측정합니다.", "실패 표본과 환경 metadata를 raw JSON에 함께 남깁니다.", "측정 race를 고친 뒤 전체를 재실행하고 synthetic 범위 안에서만 해석합니다."],
  },
