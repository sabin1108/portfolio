...
AI Agent 솔루션 페이지에서 말하는 중앙 Agent Backend, 데이터·업무 흐름·권한 연결, Human-in-the-Loop 구조도 인상 깊었습니다. AI를 단순 답변 화면으로 붙이는 것이 아니라, 조직의 데이터와 실행 흐름 안에서 신뢰할 수 있게 쓰이도록 만드는 방향이라고 이해했습니다. 이력서 두 번째 프로젝트인 AI ChatBot에서 공지·식단·일정 데이터를 채팅 UI로 연결하며 FE와 BE 책임을 나누고, 대화 내역을 서버 저장 없이 내보내게 만든 경험이 이 방향과 맞닿아 있다고 봤습니다.

Spring과 Kubernetes는 아직 실무 경험이 부족합니다. 대신 Game Information Platform에서의 Next.js·Node.js 기반 API 경계 분리와 테스트 검증, AI ChatBot에서의 응답 경계 설계, PhotoMap에서의 성능 측정과 상태 구조 개선 경험은 쌓아왔습니다. 풀링포레스트에서는 빠르게 만들되 근거 없이 밀어붙이지 않고, 문제정의와 검증 결과를 기준으로 제품을 개선하는 FullStack Developer로 기여하고 싶습니다.`;
...
AI ChatBot에서는 공지, 식단, 일정 데이터를 백엔드가 각각 다른 방식으로 수집하면서 응답 형태가 일정하지 않은 문제가 있었습니다. 프론트엔드가 이를 그대로 받으면 채팅 UI 안에 데이터별 예외 처리가 계속 늘어나는 구조라고 판단했습니다. Next.js API Routes를 응답 경계로 두고, UI 컴포넌트는 메시지 표시와 사용자 상호작용에 집중하도록 분리했습니다. localStorage로 대화 기록을 복원하고, Blob API로 대화 내역을 서버 저장 없이 내보내도록 구현했습니다. 이 프로젝트로 BRIGHT MAKERS EXPO 2025 캡스톤디자인 경진대회 우수상을 받았습니다.
    ...
    if (project.title === "AI ChatBot") {
      return {
        ...
        resolutions: [
          ...
          "액션 및 검증: useLocalStorage hook과 storage 이벤트로 대화 기록 복원과 탭 간 상태 동기화를 처리하고, Blob API로 대화 내역을 서버 저장 없이 내보내게 했습니다.",
        ],
        achievements: [
