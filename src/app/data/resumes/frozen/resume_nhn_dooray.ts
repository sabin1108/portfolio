...
export const resumeNhnDoorayData = {
  ...
  summary: [
    ...
    "협업 도구처럼 사용자가 같은 화면을 오래 반복해서 쓰는 제품에서는 기능 수보다 화면 상태의 예측 가능성과 변경 안정성이 중요하다고 생각합니다.",
    "PhotoMap에서는 Zustand selector로 구독 범위를 나누고, 대량 이미지 목록은 가상화로 보이는 영역만 렌더링하도록 정리했습니다.",
    "D3 force simulation의 tick 좌표는 React state에 올리지 않고 ref와 DOM 속성 갱신으로 분리해 복합 UI의 변경 범위를 줄였습니다.",
    "또한 고정 모바일 조건에서 이미지 전달 정책을 100회 반복 측정하며, 성능 주장이 실제 숫자와 재현 가능한 조건 위에 남도록 검증했습니다.",
    "Game Information Platform에서는 Steam, Epic, ITAD처럼 서로 다른 외부 API 응답을 공통 view model로 흡수해 화면 컴포넌트가 API별 예외를 직접 알지 않게 했습니다.",
    "0원, null, timeout, rate limit처럼 사용자 경험을 흔들 수 있는 경로는 domain rule과 Vitest, Playwright 테스트로 확인했습니다.",
    "AI는 코드를 대신 결정하는 도구가 아니라 요구사항 분해, 기존 코드 조사, 실패 테스트 작성, 반복 검증을 빠르게 만드는 도구로 사용합니다.",
    "최종 채택은 domain rule, 원시 측정값, Vitest, Playwright, production build 결과를 기준으로 판단합니다.",
    "Dooray에서도 React와 TypeScript 기반 화면을 안정적으로 운영하고, 디자인 시스템과 AI 기능이 기존 업무 흐름을 깨지 않도록 검증 가능한 프론트엔드 구조를 만드는 데 기여하고 싶습니다.",
  ].join("\n"),
